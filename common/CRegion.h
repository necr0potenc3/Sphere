...
public:
...
#if defined(_DEBUG)	// CObjBaseTemplate
	...
	virtual CObjBaseTemplate * GetTopLevelObj() const
		= 0;
...
{	// Similar to _WIN32 RECT, same as TRectMap
public:
	struct TRectMap		// map-specific rectangle
	{
		int left;
		int top;
		int right;
...
public:
    ...
    operator TRectMap &()
    {
		return( *((TRectMap*)(&m_left)) );
    }
    operator TRectMap *()
    {
		return( (TRectMap*)(&m_left) );
    }
    operator const TRectMap &() const
    {
		return( *((TRectMap*)(&m_left)) );
    }
    operator const TRectMap *() const
	...
	void SetRect( const TRectMap * pRect )
	{
		ASSERT(pRect);
		m_left = pRect->left;
		m_top = pRect->top;
	...
	CGRect & operator = ( const TRectMap * pRect )
	{
		SetRect( pRect );
		return( * this );
	}
	CGRect & operator = ( const TRectMap & rect )
	{
		SetRect( &rect );
		return( * this );
	}
	...
	void UnionRect( const CGRect & rect )
	{
		...
		if ( m_map != rect.m_map )
		{
			DEBUG_ERR(("Uniting regions from different maps!" DEBUG_CR));
		}
	}
