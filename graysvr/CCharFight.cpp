...
LPCTSTR CChar::Noto_GetFameTitle() const
{
	...
	if ( ! IsPriv( PRIV_PRIV_NOSHOW ))	// PRIVSHOW is on
	{
		...
		switch ( GetPrivLevel())
		{
			case PLEVEL_Seer: "Seer ";
			case PLEVEL_Counsel: "Counselor ";
		}
	}

	if ( Stat_GetAdjusted(STAT_FAME) > 9900 )
		return Char_GetDef()->IsFemale() ? "Lady " : "Lord ";
	return "";
}
...
#define NOTO_FACTOR 300
	...
	static UINT const sm_DegreeTable[] =
	{
...
void CChar::Noto_Kill( CChar * pKill, bool fPetKill )
{
	...
	if ( NotoThem != NOTO_GUILD_SAME )			// no fame/karma gain for killing guildmates
	{
		...
		if ( !fPetKill )							// no real fame for letting your pets do the work!
		{
			...
			{
				int change = (pKill->m_exp/10);			// default delta = exp/10 (same as death loss)
				...
				if ( change && g_Cfg.m_bLevelSystem )
				{
					int dlevel = m_level - pKill->m_level;

					if ( dlevel < 0 )				//	+10% per each lower level
						change = (change * (100 + 10 * (min(abs(dlevel),10))))/100;
					else if ( !dlevel )
						;
					else							//	-10% per each upper level
						change = (change * (100 - 10 * (min(dlevel,10))))/100;
				}
...
effect_bounce:
	...
	if ( false )
	{
		...
		CExtData cmdData;
		cmdData.DamagePacketOld.m_dmg = (iDmg < 0) ? 0 : ((iDmg > 255) ? 255 : iDmg);
		cmdData.DamagePacketOld.m_unk1 = 0;
		cmdData.DamagePacketOld.m_UID = GetUID();

		if (IsClient())
			m_pClient->addExtData(EXTDATA_DamagePacketOld, &cmdData, sizeof( cmdData.DamagePacketOld ) );
		if (pSrc->IsClient())
			pSrc->m_pClient->addExtData(EXTDATA_DamagePacketOld, &cmdData, sizeof( cmdData.DamagePacketOld ) );
	}
