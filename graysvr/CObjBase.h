...
public:
	...
	WORD	m_targX, m_targY;					// target X,Y to which the NPC was going
...
enum CTRIG_TYPE
{
	...
	CTRIG_CreatePlayer,		//+A player char have been just created
...
public:
	...
	{
		bool	fQuoted = false;
		m_TagDefs.SetStr(pszKey, fQuoted, pszVal);
	}
	...
	bool CanSeeLOS_New( const CPointMap & pd, CPointMap * pBlock = NULL, int iMaxDist = UO_MAP_VIEW_SIGHT ) const;
...
public:
	...
	void ChangeExperience(int delta = 0);
...
public:
	bool Spell_CastDone();
	bool OnSpellEffect( SPELL_TYPE spell, CChar * pCharSrc, int iSkillLevel, CItem * pSourceItem );
	bool Spell_Resurrection(CItemCorpse * pCorpse = NULL);
