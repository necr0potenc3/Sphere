...
bool CChar::NPC_SetVendorPrice( CItem * pItem, int iPrice )
{
	...
	TCHAR *pszMsg = Str_GetTemp();
	sprintf(pszMsg, g_Cfg.GetDefaultMsg(DEFMSG_NPC_VENDOR_SETPRICE_1), (LPCTSTR) pVendItem->GetName(), iPrice);
	Speak(pszMsg);
...
void CChar::NPC_PetDesert()
{
	...
	{
		TCHAR	*pszMsg = Str_GetTemp();
		sprintf(pszMsg, g_Cfg.GetDefaultMsg( DEFMSG_NPC_PET_DECIDE_MASTER ), (LPCTSTR)GetName());
		Speak(pszMsg);
