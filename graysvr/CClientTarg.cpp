...
int CClient::OnSkill_AnimalLore( CGrayUID uid, int iSkillLevel, bool fTest )
{
	...
	{
		pszText = g_Cfg.GetDefaultMsg( DEFMSG_ANIMALLORE_CONJURED );
	}
	else
	{
	...
	sprintf(pszTemp, g_Cfg.GetDefaultMsg( DEFMSG_ANIMALLORE_FOOD ), (LPCTSTR) pszHe, (LPCTSTR) pszText);
	addObjMessage(pszTemp, pChar);
...
bool CClient::OnTarg_Skill_Magery( CObjBase * pObj, const CPointMap & pt )
{
	...
	if ( IsSetOF( OF_Magic_PreCast ) && m_pChar->IsClient() )
...
bool CClient::OnTarg_Pet_Stable( CChar * pCharPet )
{
	...
	{
		SysMessageDefault( DEFMSG_NPC_STABLEMASTER_SELECT );
		return( false );
	}

	if ( ! pCharMaster->CanSee(pCharPet))
	{
		pCharMaster->Speak( g_Cfg.GetDefaultMsg( DEFMSG_NPC_STABLEMASTER_LOS ) );
		return( false );
	}
	...
	if ( pPetItem == NULL )
	{
		pCharMaster->Speak( g_Cfg.GetDefaultMsg( DEFMSG_NPC_STABLEMASTER_FAIL ) );
		return( false );
	}
	...
	sprintf(pszMsg, g_Cfg.GetDefaultMsg(DEFMSG_NPC_STABLEMASTER_REM), (LPCTSTR)pCharMaster->GetName());
...
CItem * CClient::OnTarg_Use_Multi( const CItemBase * pItemDef, const CPointMap & pt, WORD wAttr, HUE_TYPE wHue )
{
	...
	if ( pItemDef->IsType(IT_STONE_GUILD))
	{
		...
		addPromptConsole( CLIMODE_PROMPT_STONE_NAME, g_Cfg.GetDefaultMsg( DEFMSG_ITEMUSE_GUILDSTONE_NEW ) );
	}
	else if ( fShip )
	{
	...
	case IT_WEAPON_SWORD:		// 23 =
		...
		case IT_TREE:
			...
			if ( pItemUse->IsType(IT_WEAPON_FENCE) )
			{
				...
				if ( pResBit == NULL )
				{
					SysMessageDefault( DEFMSG_LUMBERJACKING_3 );
					return false;
				}
				if ( pResBit->ConsumeAmount(1) == 0 )
				{
					SysMessageDefault( DEFMSG_LUMBERJACKING_4 );
					return false;
				}
...
bool CClient::OnTarg_Party_Add( CChar * pChar )
{
	...
	if ( pChar == m_pChar )
	{
		CPartyDef::SysMessageStatic( m_pChar, "You cannot add yourself to a party" );
		return( false );
	}
	...
	*/
	...
	if ( !pChar->IsClient() )
	{
		...
		// if ( pChar->NPC_IsOwnedBy( m_pChar ) && m_pChar->CanSee( pChar ) )
		// {
		// 	CPartyDef::AcceptEvent( pChar, m_pChar->GetUID(), true );
		// 	return true;
		// }
	...
	// SysMessagef( g_Cfg.GetDefaultMsg( DEFMSG_PARTY_INVITE ), pChar->GetName() );
	// pChar->SysMessagef( g_Cfg.GetDefaultMsg( DEFMSG_PARTY_INVITE_TARG ), (LPCTSTR) m_pChar->GetName());

	// TODO: Party: Add to inviter a prop to remember the uid of the to-add member
