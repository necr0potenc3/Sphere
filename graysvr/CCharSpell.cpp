...
static LPCTSTR const sm_szPunishMsg[] =
{
	...
	{
		CRegionBase * pArea = CheckValidMove( ptNew, NULL );
		if ( pArea == NULL )
		{
...
bool CChar::Spell_Resurrection(CItemCorpse * pCorpse)
{
...
void CChar::Spell_Effect_Remove( CItem * pSpell, bool fOldStyle )
{
...
#define STAT_ADD( i, eff ) if ( fOldStyle ) Stat_AddBase( i, eff ); else Stat_AddMod( i, eff );

	switch ( spell )
	{
	case SPELL_Clumsy:
		STAT_ADD( STAT_DEX, iStatEffect );
		break;
	...
	case SPELL_Feeblemind:
		STAT_ADD( STAT_INT, iStatEffect );
		break;
	case SPELL_Weaken:
		STAT_ADD( STAT_STR, iStatEffect );
		break;
	case SPELL_Agility:
		STAT_ADD( STAT_DEX, -iStatEffect );
		break;
	...
	case SPELL_Bless:
		{
			for ( int i=STAT_STR; i<STAT_BASE_QTY; i++ )
				STAT_ADD( (STAT_TYPE) i, -iStatEffect );
		}
		break;
	...
	case SPELL_Polymorph:
		{
			...
			STAT_ADD( STAT_STR, -pSpell->m_itSpell.m_PolyStr );
			STAT_ADD( STAT_DEX, -pSpell->m_itSpell.m_PolyDex );
...
bool CChar::Spell_Equip_OnTick( CItem * pItem )
{
	...
	case SPELL_Poison:
...

		{
			...
			OnTakeDamage( max( sm_iPoisonMax[iLevel], iDmg ), pItem->m_uidLink.CharFind(), DAMAGE_POISON | DAMAGE_GENERAL );
		}
...
CItem * CChar::Spell_Effect_Create( SPELL_TYPE spell, LAYER_TYPE layer, int iSkillLevel, int iDuration, CObjBase * pSrc )
{
	...
	if (	( IsSetOF( OF_Magic_StackStats )	&&	layer == LAYER_SPELL_STATS )
		||	layer == LAYER_NONE )
...
bool CChar::Spell_CastDone()
{
	...
	if ( pObjSrc != this )
	{
		...
		if ( pItem->IsAttr( ATTR_CURSED|ATTR_CURSED2 ))
		{
			...
			pObj = this;
			SysMessageDefault( DEFMSG_SPELL_CURSED_MAGIC );
		}
	}
	...
	/*		static const ITEMID_TYPE sm_Item_Foods[] =	// possible foods.
			...
			CItem * pItem = CItem::CreateScript( sm_Item_Foods[ Calc_GetRandVal( COUNTOF( sm_Item_Foods )) ], this );
...
int CChar::Spell_CastStart()
{
	...
	//  0-100
	//  -1 = instant failure.
	if ( ! IsSetOF( OF_Magic_PreCast ) || !IsClient() )
	{
		if ( ! Spell_TargCheck())
			return( -1 );
	}
	...
	if ( fWOP )
	{
		...
		{
			Speak( ((LPCTSTR) pSpellDef->m_sRunes) + 1 );
		}
		else
		{
			int i=0;
	...
	if ( IsPriv( PRIV_GM ))
		...
		iWaitTime = pSpellDef->m_CastTime.GetLinear( Skill_GetBase( (SKILL_TYPE) iSkill ) );
...
bool CChar::OnSpellEffect( SPELL_TYPE spell, CChar * pCharSrc, int iSkillLevel, CItem * pSourceItem )
{
	...
	if ( pSpellDef->IsSpellType( SPELLFLAG_HARM ))
	{
		...
		if (	!IsSetOF( OF_Magic_CanHarmSelf )
			&&	pCharSrc == this )
			return( false );
...
reflectit:
	...
	case SPELL_Harm:
		OnTakeDamage( GetSpellEffect( spell, iSkillLevel, iEffectMult ), pCharSrc, DAMAGE_MAGIC | DAMAGE_POISON | DAMAGE_GENERAL );
		break;
	case SPELL_Mind_Blast:
		OnTakeDamage( GetSpellEffect( spell, iSkillLevel, iEffectMult ), pCharSrc, DAMAGE_MAGIC | DAMAGE_POISON | DAMAGE_GENERAL );
		break;
	case SPELL_Explosion:
		OnTakeDamage( GetSpellEffect( spell, iSkillLevel, iEffectMult ), pCharSrc, DAMAGE_MAGIC | DAMAGE_HIT_BLUNT | DAMAGE_GENERAL );
		break;
	case SPELL_Energy_Bolt:
	case SPELL_Magic_Arrow:
		OnTakeDamage( GetSpellEffect( spell, iSkillLevel, iEffectMult ), pCharSrc, DAMAGE_MAGIC | DAMAGE_HIT_PIERCE );
		break;
	case SPELL_Fireball:
	case SPELL_Fire_Bolt:
		OnTakeDamage( GetSpellEffect( spell, iSkillLevel, iEffectMult ), pCharSrc, DAMAGE_MAGIC | DAMAGE_HIT_BLUNT | DAMAGE_FIRE );
		break;
	case SPELL_Fire_Field:
	case SPELL_Flame_Strike:
		// Burn whoever is there.
		OnTakeDamage( GetSpellEffect( spell, iSkillLevel, iEffectMult ), pCharSrc, DAMAGE_MAGIC | DAMAGE_FIRE | DAMAGE_GENERAL );
		break;
	case SPELL_Meteor_Swarm:
	...
	case SPELL_Chain_Lightning:
		...
		OnTakeDamage( GetSpellEffect( spell, iSkillLevel, iEffectMult ), pCharSrc, DAMAGE_MAGIC | DAMAGE_ELECTRIC | DAMAGE_GENERAL );
		break;
	...
	case SPELL_Light:
		...
		Spell_Effect_Create( spell, fPotion ? LAYER_FLAG_Potion : LAYER_LIGHT, iSkillLevel,
			GetSpellDuration( spell, iSkillLevel, iEffectMult ), pCharSrc );
		break;
...
#define SPELL_MAX_POLY_STAT 150
	...
	case SPELL_Forget:			// 109 // permanently lowers one skill.
		{
			int iSkillLevel = 0;
			Skill_Degrade((SKILL_TYPE) MAX_SKILL);
		}
		break;
