...
bool CContainer::r_WriteValContainer( LPCTSTR pszKey, CGString & sVal )
{
	...
	int i,len;
	for ( i = 0; i < COUNTOF(sm_szParams); i++ )
	{
		len = strlen(sm_szParams[i]);
		if ( !strnicmp(pszKey, sm_szParams[i], len) )
		{
			LPCTSTR	pKey = pszKey += len;
			SKIP_SEPERATORS(pKey);
			if ( !*pszKey || ISWHITESPACE(*pKey) )
			{
				switch ( i )
				{
				case 0:			//	rescount
					{
						if ( ! *pKey ) sVal.FormatVal(GetCount());
						else sVal.FormatVal(ContentCount(g_Cfg.ResourceGetID(RES_ITEMDEF, pKey)));
						return true;
					}
				case 1:			//	restest
					{
