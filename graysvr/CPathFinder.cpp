...
void CPathFinder::FillMap()
{
	int x, y;
	CRegionBase	*pArea;
	CPointMap	pt = m_pChar->GetTopPoint(), ptChar = m_pChar->GetTopPoint();
	...
	for ( x = 0; x < PATH_SIZE; x++ )
	{
		for ( y = 0; y < PATH_SIZE; y++ )
		{
			...
			if ( !pArea )
			{
//				DEBUG_ERR(("ADVAI: %d,%d is unwalkable!" DEBUG_CR, pt.m_x, pt.m_y));
				m_walkability[x][y] = PATH_UNWALKABLE;
			}
			else m_walkability[x][y] = PATH_WALKABLE;
		}
	}
}
...
void CPathFinder::ReadPath(int currentX, int currentY, int pid)
{
	if ( m_pathStatus[pid] == PATH_FOUND )
	{
		m_xPath[pid] = ReadPathX(m_pathLocation[pid]);
		m_yPath[pid] = ReadPathY(m_pathLocation[pid]);
	}
	else
	{	
		m_xPath[pid] = currentX;
		m_yPath[pid] = currentY;
	}
}
