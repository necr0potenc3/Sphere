...
private:
	...
	void Event_Spy2( const BYTE * pEvent );
	...
	void Event_ViewRange( const BYTE range );
...
public:
	...
	char	m_PacketFilter[XCMD_QTY][32];	// list of packet filtering functions
...
extern class CClientTask : public CThread
{
	// The client events thread. (handles all client connections)
private:
	CServTime m_timeRestart;
	CServTime m_timeWarn;
	CServTime m_timePrev;
...
//	NPC AI events thread (pathfinding, some more actions)
extern class CAITask : public CThread
{
private:
	CServTime m_timeRestart;
	CServTime m_timeWarn;
...
//	Exceptions debugging routine.
	...
	#define EXC_SET(a)	inLocalBlock = a; \
						inLocalBlockCnt++
	#define EXC_CATCH_EXCEPTION(a,b) \
							if ( inLocalBlock ) \
								g_Log.CatchEvent(a, "%s (%i,%s) - %s", inLocalArgs, inLocalBlockCnt, inLocalBlock, b); \
							else \
								g_Log.CatchEvent(a, "%s - %s", inLocalArgs, b)
	#define EXC_CATCH_NOTHROW(a)	}	\
							catch ( CGrayError &e )	\
							{ \
								EXC_CATCH_EXCEPTION(&e, a); \
							} \
							catch (...) \
							{ \
								EXC_CATCH_EXCEPTION(NULL, a); \
							}
