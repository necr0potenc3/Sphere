...
{
	m_fDebugService = m_fIsNTService = true;

	// Set the Console Control Handler so that the application will respond to ^C/^Break from console
...
do_not_nt_service:
	...
	if ( argc && _IS_SWITCH(*argv[1]) )
	{
		char *next = argv[1] + 1;
		if ( !strcmp(next, "install") )
		{
			g_Service.ReportEvent(EVENTLOG_INFORMATION_TYPE, 0, "Installing Service.");
			g_Service.CmdInstallService();
			return 0;
		}
		else if ( !strcmp(next, "remove") )
		{
			g_Service.ReportEvent(EVENTLOG_INFORMATION_TYPE, 0, "Removing Service.");
			g_Service.CmdRemoveService();
			return 0;
		}
		else if ( !strcmp(next, "debug") )
		{
			g_Service.ReportEvent(EVENTLOG_INFORMATION_TYPE, 0, "Debugging Service.");
			goto do_not_nt_service;
		}
	}
