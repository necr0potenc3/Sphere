...
void CItem::r_Write( CScript & s )
{
	...
	if ( IsSetEF(EF_Size_Optimise) ) s.WriteSection("WI %s", GetResourceName());
	else s.WriteSection("WORLDITEM %s", GetResourceName());
...
bool CItem::r_WriteVal( LPCTSTR pszKey, CGString & sVal, CTextConsole * pSrc )
{
	...
	case IC_MOREY:
		...
		break;
	case IC_MOREZ:
		if ( IsType(IT_SPELLBOOK) || IsType(IT_SPELLBOOK_NECRO) || IsType(IT_SPELLBOOK_PALA) || IsType(IT_SPELLBOOK_EXTRA) )
...
TRIGRET_TYPE CItem::OnTrigger( LPCTSTR pszTrigName, CTextConsole * pSrc, CScriptTriggerArgs * pArgs )
{
	...
	{
		g_Log.CatchEvent( &e, "Item Trigger in %s (%s) (UID=0%lx)", pszTrigName, zTemp, (DWORD) GetUID());
	}
	catch (...)	// catch all
	{
		g_Log.CatchEvent(NULL, "Item Trigger in %s (%s) (UID=0%lx)", pszTrigName, zTemp, (DWORD) GetUID());
	}
#endif
...
LPCTSTR CItem::Use_Sextant( CPointMap pntCoords ) const
{
	...
	long lLat =  (pntCoords.m_y - g_pntLBThrone.m_y) * 360 * 60 / g_MapList.GetY(pntCoords.m_map);
	long lLong;
	if ( pntCoords.m_map <= 1 )
		lLong = (pntCoords.m_x - g_pntLBThrone.m_x) * 360 * 60 / UO_SIZE_X_REAL;
	else
		lLong = (pntCoords.m_x - g_pntLBThrone.m_x) * 360 * 60 / g_MapList.GetX(pntCoords.m_map);
	int iLatDeg =  lLat / 60;
	int iLatMin  = lLat % 60;
...
bool CItem::SetBlessCurse( int iSkillLevel, CChar * pCharSrc )
{
	...
	sprintf(pszMsg, "show%s a %s glow.", IsItemEquipped() ? "" : "s", pszColor);
	Emote(pszMsg);
	return true;
}
