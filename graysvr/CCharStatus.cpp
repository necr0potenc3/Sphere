...
CItem * CChar::GetSpellbook( SPELL_TYPE iSpell ) const
{
	...
	{
		if ( pBook->IsType(IT_SPELLBOOK) || pBook->IsType(IT_SPELLBOOK_NECRO) || 
			pBook->IsType(IT_SPELLBOOK_PALA) || pBook->IsType(IT_SPELLBOOK_EXTRA) )
		{
...
{
	if ( IsSetOF(OF_Advanced_LOS) )
	{
		// g_Log.Event( LOGL_WARN, "CanSeeLOS NEW" DEBUG_CR);
...
bool CChar::CanSeeLOS_New( const CPointMap & ptDst, CPointMap * pptBlock, int iMaxDist ) const
{
	if ( IsPriv( PRIV_GM ))
	{
		// g_Log.Event( LOGL_WARN, "GM Pass" DEBUG_CR);
		return( true );
	}
...
nolos:
	...
	if ( IsHuman() )
	{
		// g_Log.Event( LOGL_WARN, "LOS: Human" DEBUG_CR);
		ptSrc.m_z += (PLAYER_HEIGHT - 3); // Eye height
		if ( IsStatFlag(STATF_OnHorse) )
		{
			// g_Log.Event( LOGL_WARN, "LOS: Human Mounted" DEBUG_CR);
			ptSrc.m_z += 10; // Mounted player?
		}
	}
	else // TODO: Well i have no clue for determining height of Npcs/Morphed players. Adding a def for npcs??
	{
		// g_Log.Event( LOGL_WARN, "LOS: Random (min: 2)" DEBUG_CR);
		ptSrc.m_z += ((rand()%PLAYER_HEIGHT) + 2);
	}
	...
	dist2d = sqrt(pow(dx,2)+pow(dy,2));
	if (dz)
		dist3d = sqrt(pow(dist2d,2)+pow(dz,2));
	else
		dist3d = dist2d;
	...
	while ( true )
	{
		if ( BETWEENPOINT(nPx,ptDst.m_x,ptSrc.m_x) && BETWEENPOINT(nPy,ptDst.m_y,ptSrc.m_y) && BETWEENPOINT(nPz,ptDst.m_z,ptSrc.m_z) )
		{
			...
			if (path.GetSize())
			{
				CPointMap ptEnd = path.GetAt(path.GetSize()-1);
	...
	if ( path.GetSize() )
	{
		if ( path.GetAt(path.GetSize()-1) != ptDst ) 
	...
	// g_Log.Event( LOGL_WARN, "Path calculated %d" DEBUG_CR, path.GetSize());
	...
	for(int i = 0; i < path.GetSize(); lp_x = ptNow.m_x, lp_y = ptNow.m_y, pItemDef = NULL, pStatic = NULL, pMulti = NULL, pMultiItem = NULL, min_z = 0, max_z = 0, i++)
	{
		ptNow = path.GetAt(i);
		// g_Log.Event( LOGL_WARN, "Point %d,%d,%d" DEBUG_CR, ptNow.m_x,ptNow.m_y,ptNow.m_z);
		...
		{
			// g_Log.Event( LOGL_WARN, "GetMapBlock Failed" DEBUG_CR);
			bPath = false;
			break;
		}
		...
		// g_Log.Event( LOGL_WARN, "Terrain %d" DEBUG_CR, terrainid);
		...
		if ( terrainid != TERRAIN_HOLE && terrainid != 475 )
		{
			if ( terrainid < 430 || terrainid > 437 )
			{
				min_z = MAPTILEMIN;
				max_z = MAPTILEMAX;
				// g_Log.Event( LOGL_WARN, "Terrain %d - m:%d M:%d" DEBUG_CR, terrainid, min_z, max_z);
				if (min_z <= ptNow.m_z && max_z >= ptNow.m_z && (ptNow.m_x != ptDst.m_x || ptNow.m_y != ptDst.m_y  || min_z > ptDst.m_z || max_z < ptDst.m_z))
				{
					// g_Log.Event( LOGL_WARN, "Terrain %d - m:%d M:%d - block" DEBUG_CR, terrainid, min_z, max_z);
					bPath = false;
					break;
				}
			}
		...
		for ( int s = 0; s < pBlock->m_Statics.GetStaticQty(); pStatic = NULL, pItemDef = NULL, s++ )
		{
			...
			if ( pItemDef->GetTFlags() & 12288 )
			{
			// g_Log.Event(LOGL_WARN,"pStatic %0x %d,%d,%d - %d"DEBUG_CR,pStatic->GetDispID(),pStatic->m_x,pStatic->m_y,pStatic->m_z,pItemDef->GetHeight());
				min_z = pStatic->m_z;
				max_z = min_z;
				if ( pItemDef->GetTFlags() & 1024 )
					max_z += (pItemDef->GetHeight() / 2);
				else
					max_z += pItemDef->GetHeight();
				if (min_z <= ptNow.m_z && max_z >= ptNow.m_z && (ptNow.m_x != ptDst.m_x || ptNow.m_y != ptDst.m_y  || min_z > ptDst.m_z || max_z < ptDst.m_z))
				{
					// g_Log.Event( LOGL_WARN, "pStatic blocked - m:%d M:%d" DEBUG_CR, min_z,max_z);	
					bPath = false;
					break;
				}
			}
...
CRegionBase * CChar::CheckValidMove( CPointBase & ptDest, WORD * pwBlockFlags, DIR_TYPE dir ) const
{
	...
	if ( IsSetEF( EF_DiagonalWalkCheck )	&& (dir % 2) )		// test only diagonal dirs
	{
		CPointMap	ptTest;
		DIR_TYPE	dirTest1	= (DIR_TYPE) (dir-1);	// get 1st ortogonal
		DIR_TYPE	dirTest2	= (DIR_TYPE) (dir+1);	// get 2nd ortogonal
	...
	if ( pArea == NULL )
	{
		g_Log.Event( LOGL_WARN, "MOVE: UID(%0x) is moving into an empty region (%d,%d,%d,%d)" DEBUG_CR, (DWORD)GetUID(), ptDest.m_x, ptDest.m_y, ptDest.m_z, ptDest.m_map );
		return( NULL );
	}
